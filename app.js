...
function renderChat() {
    ...
    chat.messages.forEach((msg, idx) => {
        ...
        messageDiv.innerHTML = `<div class="avatar">${avatarHTML}</div><div class="message-content-wrapper"><div class="message-content">${rawContent}</div><div class="message-time">${formatTime(msg.timestamp || Date.now())}</div></div>`;
...
function addMessageToDOM(role, content, save=true) {
    const timestamp = Date.now(); const messageId = timestamp+Math.random();
    if(save) {
        const chat = chats.find(c=>c.id===currentChatId);
        if(chat) {
...
async function sendMessage() {
    ...
    else addMessageToDOM('assistant', '❌ Не удалось получить ответ. Проверьте ключ или повторите позже.', true);
...
function setupFoldersUI() {
    ...
    ].map(icon=>`<div class="icon-option" data-icon="${icon}"><i class="fas ${icon}"></i></div>`).join('');
    ...
    if(colorSel) document.querySelectorAll('.color-option').forEach(opt => opt.onclick = () => {
...
function setupEventListeners() {
    ...
    document.getElementById('dropdown-logout')?.addEventListener('click', ()=>{
        userApiKey=''; localStorage.removeItem('diamond_api_key');
    ...
    if(avatarContainer) avatarContainer.onclick = () => {
        ...
        document.querySelectorAll('.avatar-icon').forEach(icon => {
            if(userAvatar.type==='icon' && userAvatar.value===icon.dataset.icon) icon.classList.add('selected');
